...
AsmError Assembler(FILE* file_to_bin, FILE* listing, String* text, Text* data, Label* label, VAssembler* ass) 
   ...
   for (size_t Nstring = 0; Nstring < data->count_n; Nstring++)
        {
        if (data->str[data->count_n - 1].length == 0  && data->count_n - 1 == Nstring)
            break;
...
AsmError ParseArgument(FILE* listing, char arg[], VAssembler* ass, Label* label, String* text, size_t Nstring, size_t Ncomand)
    ...
    if (sscanf(arg, "%lf", &DoubleArg) == 1) 
        ...
        if (!plus)
            ...
            {
            ass->machine_code |= IMM | REG | RAM;

            second_arg = atoi(plus);
            }
        
        FillBufferWithReg(StrArg, second_arg, ass, listing, Nstring, Ncomand);
        } 
        ...
        if (IsReg(StrArg, &IntArg)) 
            ...
            FillBufferWithReg(StrArg, 0, ass, listing, Nstring, Ncomand); 
            }
    ...
    ass->Buffer[saved_ip] = ass->machine_code;
    
    return SUCCESS;
    }


AsmError FillBufferWithReg(char StrArg[], int second_arg, VAssembler* ass, FILE* listing, size_t Nstring, size_t Ncomand) 
    {
    int arg = 0;

    if (IsReg(StrArg, &arg)) 
        {
        *(ass->Buffer + ass->ip + 1) = arg;

        if (second_arg != 0) 
            {
            *(double*)(ass->Buffer + ass->ip + 2) = (double)second_arg;
